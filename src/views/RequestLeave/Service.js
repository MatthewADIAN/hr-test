...
class Service {
    ...
    searchEmployee = (params) => {
        ...
        let isLeadEmployee = params.isLeadEmployee;
        ...
        if (isLeadEmployee) {
          if (query === '') {
              query = `keyword=${isLeadEmployee}`
          } else {
              query = `${query}&isLeadEmployee=${isLeadEmployee}`
          }
        }
    ...
    createRequestLeave = (payload) => {
        console.log(payload)
        let url = `${CONST.URI_ATTENDANCE}request-leave/`;
        return axios.post(url, payload, { headers: HEADERS })
            .then((result) => {
                return result.data;
            })
    ...
    editRequestLeave = (id, payload) => {
        let url = `${CONST.URI_ATTENDANCE}request-leave/${id}`;
        return axios.put(url, payload, { headers: HEADERS })
            .then((result) => {
                return result.data;
            })
    ...
    getEmployeeByEmployeeIdentity = (employeeIdentity) => {
        let url = `${CONST.URI_ATTENDANCE}employees/by-employee-identity/${employeeIdentity}`;
        return axios.get(url, { headers: HEADERS })
            .then((result) => {
                return result.data;
            })
    ...
    getLeaveTypebyRequest= (params) => {
        ...
        let employeeId = params.employeeId || 0;
        let isShown = true;
        let url = `${CONST.URI_ATTENDANCE}leave-types/show-by-request/?employeeId=${employeeId}&isShown=${isShown}`;

        return axios.get(url, { headers: HEADERS })
            .then(data => {
                var leaveTypes = data.data.map((datum) => {
                    datum.value = datum.Id;
                    datum.label = datum.Name;
                    return datum;
                });
    ...
    getRequestLeave = (params) => {
        ...
        let page = params.page || 1;
        let size = params.size || 10;
        let url = `${CONST.URI_ATTENDANCE}request-leave/?startDate=${startDate}&endDate=${endDate}&unitId=${unitId}&employeeId=${employeeId}&page=${page}&size=${size}&adminEmployeeId=${adminEmployeeId}`;
    ...
    getRequestLeaveByEmployeeIdentity = (params) => {
        let unitId = params.unitId || 0;
        let employeeIdentity = params.employeeIdentity || "";
        let startDate = params.startDate && params.startDate != "Invalid date" ? moment(params.startDate).format("YYYY-MM-DD") : moment().add(-10, 'day').format("YYYY-MM-DD");
        let endDate = params.endDate && params.endDate != "Invalid date" ? moment(params.endDate).format("YYYY-MM-DD") : moment().add(10, 'day').format("YYYY-MM-DD");
        let adminEmployeeId = Number(localStorage.getItem("employeeId"));
        ...
        let page = params.page || 1;
        let size = params.size || 10;
        let url = `${CONST.URI_ATTENDANCE}request-leave/by-employee-identity/?startDate=${startDate}&endDate=${endDate}&unitId=${unitId}&employeeIdentity=${employeeIdentity}&adminEmployeeId=${adminEmployeeId}&page=${page}&size=${size}`;
    ...
    getOvertimeReport = (params) => {
        ...
        let page = params.page || 1;
        let size = params.size || 10;
        let url = `${CONST.URI_ATTENDANCE}report-overtime?startDate=${startDate}&endDate=${endDate}&unitId=${unitId}&sectionId=${sectionId}&groupId=${groupId}&page=${page}&size=${size}`;
    ...
    getNotAttendReport = (params) => {
        ...
        let url = `${CONST.URI_ATTENDANCE}report-not-attend?date=${date}&unitId=${unitId}&sectionId=${sectionId}&groupId=${groupId}&flag=${flag}&page=${page}&size=${size}`;
    ...
    getEditAllAbsensiReport = (params) => {
        ...
        let unitId = params.unitId || 0;
        let date = params.date && params.date != "Invalid date" ? moment(params.date).format("YYYY-MM-DD") : null;
        let url = `${CONST.URI_ATTENDANCE}attendances/report/edit-all?date=${date}&unitId=${unitId}&sectionId=${sectionId}&groupId=${groupId}`;
    ...
    getAttendance = (params) => {
        ...
        let url = `${CONST.URI_ATTENDANCE}attendances/?startDate=${startDate}&endDate=${endDate}&unitId=${unitId}&sectionId=${sectionId}&groupId=${groupId}&page=${page}&size=${size}&employeeId=${employeeId}`;

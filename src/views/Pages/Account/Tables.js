...
class Tables extends Component {
  ...
  handleGetUser = () => {
    ...
    axios({
      method: 'get',
      url: this.state.url + `?page=1&order=%7B%7D&filter=%7B%7D&size=${this.state.resultsPerPage}`,
      headers: Header,
    })
      .then(data => {
        var i;
        ...
        <tr key={results._id} data-category={results._id}>
          ...
          <td>
            ...
            <Button
              ...
              onClick={() => this.handleDetails(results.EmployeeIdentity)}>
              Details
            </Button>
            &nbsp;
            <Button
              className="btn btn-danger"
              onClick={() => this.deleteAccount(results.EmployeeIdentity)}>
              Delete
            </Button>
          </td>
        </tr>
